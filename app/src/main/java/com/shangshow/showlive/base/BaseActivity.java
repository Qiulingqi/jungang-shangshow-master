...
public abstract class BaseActivity extends IActivity implements SwipeBackActivityBase {
    ...
    protected void setContentViewOption(int resId, boolean hasTitle, boolean hasLoadingOverLayView) {
        if (hasTitle && hasLoadingOverLayView) {
            ViewGroup root = (ViewGroup) LayoutInflater.from(this).inflate(R.layout.activity_container_title_state, null);
            ((ViewGroup) (root.findViewById(R.id.mContent))).addView(LayoutInflater.from(this).inflate(resId, null), 0);
